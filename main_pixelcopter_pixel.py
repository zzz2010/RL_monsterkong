...
def get_env_obs(ple_env,last_obs=None):
    ...
    X=np.arange(0,obs.shape[0])[:,np.newaxis].repeat(obs.shape[1],axis=1)
    Y = np.arange(0, obs.shape[1])[np.newaxis,:].repeat(obs.shape[0], axis=0)
    if last_obs is not None:
        return np.concatenate([X[np.newaxis, :],Y[np.newaxis, :],last_obs[-1:, :],obs[np.newaxis, :]])
    else:
        return np.concatenate([X[np.newaxis, :],Y[np.newaxis, :],obs[np.newaxis, :],obs[np.newaxis, :]])
